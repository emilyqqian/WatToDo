...
# Backend Setup Guide (To‑Do App)

The following instructions will (hopefully) help you set up the development environment for the backend of the To‑Do App project.

If not, ask ChatGPT. **Don't ask me. I spent 2 hours trying to compile my code.**
...
* **mysqlcppconn (Version 9.5)**
* **CMake**
* **Crow** — *A lightweight C++ web framework. It provides Flask‑like route handling, JSON support, middleware, and a built‑in HTTP server. We use Crow as the foundation of our backend API to handle requests, send responses, and organize routes cleanly.*

---

## Database Setup

After connecting to a database, run the script located at:

```
src/databaseScheme.sql
```

This sets up all required tables.

### Notes:

* Only run this if you **do not already have your database set up**.
* **Project‑Team 17:** *Do NOT run this script,* as it will **reset the entire database**.
